...
public class MazeGrid : ScriptableObject
{
    ...
    public Float2 GetCoordinatesAt(Directions dir)
    {
        Float2 vector = dir.DirectionToMatrix() * (Length - 1);
        return new((int)vector.x, (int)vector.y);
    }

    public Vector3 CoordinateToPosition(Float2 coord) => new(GetHalfPoint(Size.x, coord.x), GetHalfPoint(Size.y, coord.y));
